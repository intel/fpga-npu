...
template<typename T>
int linux_read_multi(ssize_t fd, void *addr, T *data_ptr,size_t data_cnt)
{
    ssize_t result;
    result = pread(fd, data_ptr, data_cnt,
                   reinterpret_cast<off_t>(addr));
    return result == data_cnt;
}

template<typename T>
int linux_write_multi(ssize_t fd, void *addr, T *data_ptr,size_t data_cnt)
{
    ssize_t result;
    result = pwrite(fd, data_ptr, data_cnt,
                    reinterpret_cast<off_t>(addr));
    return result == data_cnt;
}
...
int intel_fpga_pcie_dev::read256(void *addr, void *data_ptr)
{
    return linux_read_multi(m_dev_handle, addr, data_ptr,32);
}

int intel_fpga_pcie_dev::read512(void *addr, void *data_ptr)
{
    return linux_read_multi(m_dev_handle, addr, data_ptr,64);
}
...
int intel_fpga_pcie_dev::write256(void *addr, void* data)
{
    return linux_write_multi(m_dev_handle, addr, data,32);
}

int intel_fpga_pcie_dev::write512(void *addr, void* data)
{
    return linux_write_multi(m_dev_handle, addr, data,64);
}
